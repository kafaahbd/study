    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-800 py-8 px-4 transition-colors">
      ...
      <div className="max-w-4xl mx-auto flex items-center justify-between mb-8 print:hidden">
        ...
        <div className="flex gap-3">
          ...
          >
            <i className={isDownloading ? "fas fa-spinner animate-spin" : "fas fa-file-download"}></i>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-4 mb-10">
          <div className="bg-green-50 dark:bg-green-900/20 p-6 rounded-2xl text-center border border-green-100 dark:border-green-800">
            <p className="text-sm font-bold text-green-600 uppercase mb-1">{lang === "bn" ? "সঠিক" : "Correct"}</p>
            <p className="text-4xl font-black text-green-700 dark:text-green-400">{result.correct} / {result.total}</p>
          </div>
          <div className="bg-blue-50 dark:bg-blue-900/20 p-6 rounded-2xl text-center border border-blue-100 dark:border-blue-800">
            <p className="text-sm font-bold text-blue-600 uppercase mb-1">{lang === "bn" ? "স্কোর" : "Score"}</p>
        ...
        <div className="space-y-12">
          {result.results.map((item: any, idx: number) => {
            const userOptionText = item.options[item.userAnswer] || "Not Answered";
              ...
              <div key={idx} className="group">
                ...
                <div className="ml-12 space-y-3">
                  ...
                  <div className={`p-4 rounded-xl flex items-center gap-3 ${
                    item.isCorrect ? "bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300" : "bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300"
                  }`}>
                    <i className={`fas ${item.isCorrect ? "fa-check-circle" : "fa-times-circle"}`}></i>
                    <p className="font-medium">
                      <span className="font-bold">{lang === "bn" ? "আপনার উত্তর: " : "Your Answer: "}</span>
                  ...
                  {!item.isCorrect && (
                    <div className="p-4 rounded-xl bg-gray-50 dark:bg-gray-700/50 text-gray-700 dark:text-gray-200 border border-dashed border-gray-300 dark:border-gray-600">
                      <p className="font-medium">
                        <span className="font-bold text-green-600">{lang === "bn" ? "সঠিক উত্তর: " : "Correct: "}</span>
