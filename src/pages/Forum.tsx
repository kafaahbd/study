...
const Forum: React.FC = () => {
	...
	// fetchPosts ফাংশনটি আপডেট করে
	const fetchPosts = async () => {
		try {
			// ব্যাকএন্ডে ক্যাটাগরি এবং ব্যাচ পাঠানো (যদি ব্যাকএন্ড সাপোর্ট করে)
	...
	useEffect(() => {
		fetchPosts();
	}, [activeCategory, activeBatch]); // ফিল্টার চেঞ্জ হলে নতুন করে ডেটা আসবে
	...
	const filteredPosts = useMemo(() => {
		return posts.filter((post) => {
			...
			const batchMatch = activeBatch === "All" || post.batch === activeBatch;
			return categoryMatch && batchMatch;
		});
	}, [posts, activeCategory, activeBatch]);
	...
	return (
		<div className="max-w-3xl mx-auto py-10 px-4 min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-500">
						...
						<div className="flex gap-3">
							...
							<div className="relative">
								...
								>
									...
									{batch}
									<motion.div animate={{ rotate: isBatchOpen ? 180 : 0 }}>
										<ChevronDown size={14} />
									</motion.div>
								</button>
								<AnimatePresence>
			...
			{/* --- Filter Bar (Professional Sticky) --- */}
			<div className="sticky top-4 z-[100] mb-8">
				<div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-3xl shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-gray-700 p-2 overflow-x-auto no-scrollbar">
					<div className="flex items-center gap-2 min-w-max">
						<div className="p-2 ml-2 text-blue-600 dark:text-blue-400">
							<Filter size={18} />
						</div>
						...
						<div className="flex gap-1.5 border-r border-gray-100 dark:border-gray-700 pr-3 mr-1">
							{filterCategories.map((cat) => (
								<button
									...
									className={`px-4 py-1.5 rounded-2xl text-[11px] font-black transition-all ${activeCategory === cat ? "bg-blue-600 text-white shadow-lg shadow-blue-500/30" : "bg-gray-50 dark:bg-gray-900 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"}`}
								>
									{cat}
								</button>
							))}
						...
						<div className="flex gap-1.5">
							{batches.map((b) => (
								<button
									...
									className={`px-4 py-1.5 rounded-2xl text-[11px] font-black transition-all ${activeBatch === b ? "bg-indigo-600 text-white shadow-lg shadow-indigo-500/30" : "bg-gray-50 dark:bg-gray-900 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"}`}
								>
									{b}
								</button>
							))}
					...
					{filteredPosts.length > 0 ? (
						filteredPosts.map((post) => (
							...
							>
									...
									<div className="flex gap-4">
										...
										<div>
											...
											<div className="flex items-center gap-2 mt-2">
												<span className="text-[9px] font-black text-blue-500 bg-blue-50 dark:bg-blue-900/20 px-2 py-0.5 rounded-md uppercase">
													{post.category}
												</span>
												<span className="text-[9px] font-black text-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 px-2 py-0.5 rounded-md uppercase">
													{post.batch}
												</span>
											</div>
										</div>
								...
								<p className="text-gray-700 dark:text-gray-300 text-[16px] leading-relaxed mb-7 font-medium whitespace-pre-wrap">
									{post.content}
								</p>
								<div className="flex items-center gap-8 pt-6 border-t border-gray-50 dark:border-gray-700/30">
									...
									<button
										...
										className="text-gray-300 hover:text-blue-500 ml-auto p-1 transition-colors"
									>
										<Share2 size={20} />
									</button>
								</div>
