...
import { MessageSquare, Heart, Share2, Inbox, Trash2, Send, Layers, GraduationCap, ChevronDown, Check } from 'lucide-react';
...
const Forum: React.FC = () => {
  ...
  return (
    <div className="max-w-3xl mx-auto py-10 px-4 min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-500">
      ...
      {/* Post Box */}
      <motion.div className="bg-white dark:bg-gray-800 rounded-[45px] shadow-2xl shadow-blue-500/5 border border-white dark:border-gray-700/50 p-3 mb-12">
        <div className="bg-gray-50/50 dark:bg-gray-900/40 rounded-[40px] p-6">
          ...
          <div className="flex flex-wrap items-center justify-between mt-4 pt-4 border-t border-gray-100 dark:border-gray-700/50 gap-4">
            <div className="flex gap-3">
              <div className="relative">
                <button 
                  ...
                  className="flex items-center gap-2 bg-white dark:bg-gray-800 px-4 py-2.5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 text-[11px] font-black dark:text-gray-200 transition-all"
                >
                ...
                <AnimatePresence>
                  {isCatOpen && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 5 }} exit={{ opacity: 0, y: 10 }} className="absolute left-0 top-full z-[120] w-40 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 overflow-hidden p-1">
                      {categories.map((cat) => (
                        <button key={cat} onClick={() => { setCategory(cat); setIsCatOpen(false); }} className={`flex items-center justify-between w-full px-4 py-2.5 text-[11px] font-bold rounded-xl ${category === cat ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600' : 'text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>{cat} {category === cat && <Check size={12} />}</button>
                      ))}
                    </motion.div>
                  )}
                </AnimatePresence>
              ...
              <div className="relative">
                ...
                <AnimatePresence>
                  {isBatchOpen && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 5 }} exit={{ opacity: 0, y: 10 }} className="absolute left-0 top-full z-[120] w-40 bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-gray-700 overflow-hidden p-1">
                      {batches.map((b) => (
                        <button key={b} onClick={() => { setBatch(b); setIsBatchOpen(false); }} className={`flex items-center justify-between w-full px-4 py-2.5 text-[11px] font-bold rounded-xl ${batch === b ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600' : 'text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>{b} {batch === b && <Check size={12} />}</button>
                      ))}
                    </motion.div>
                  )}
                </AnimatePresence>
            ...
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handlePostSubmit} className="bg-blue-600 text-white px-8 py-3 rounded-2xl text-[13px] font-black shadow-lg shadow-blue-500/30 flex items-center gap-2">
              <Send size={16} /> Post
            </motion.button>
          </div>
        </div>
      ...
      <div className="space-y-8">
        {posts.length > 0 ? posts.map((post) => (
          <motion.div layout key={post.id} className="bg-white dark:bg-gray-800 rounded-[40px] shadow-sm border border-gray-100 dark:border-gray-700 p-8 group">
            <div className="flex justify-between items-start mb-6">
              ...
              {post.user_id === user?.id && (
                <button onClick={() => setDeleteModal({ isOpen: true, postId: post.id })} className="text-red-400 p-2 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-all"><Trash2 size={18} /></button>
              )}
            </div>
            ...
            <div className="flex items-center gap-8 pt-6 border-t border-gray-50 dark:border-gray-700/30">
              <button onClick={() => handleToggleReact(post.id)} className={`flex items-center gap-2 text-sm font-black transition-all ${post.is_reacted ? 'text-red-500' : 'text-gray-400'}`}>
                <Heart size={22} fill={post.is_reacted ? "currentColor" : "none"} /> {post.react_count || 0}
              </button>
              <button onClick={() => navigate(`/post/${post.id}`)} className="flex items-center gap-2 text-sm font-black text-gray-400 hover:text-blue-600 transition-colors">
                <MessageSquare size={22} /> {post.comment_count || 0}
              </button>
              <button onClick={() => handleShare(post)} className="text-gray-300 hover:text-blue-500 ml-auto p-1 transition-colors"><Share2 size={20} /></button>
            </div>
          </motion.div>
        )) : (
